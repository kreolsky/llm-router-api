...
from src.api import stat_routes
...
class TestStatUsageDays:
    ...
    async def test_non_numeric_days_raises_422_envelope_not_500(self):
        with pytest.raises(HTTPException) as exc_info:
            await stat_routes.stat_usage(users="", models="", days="abc", _=None)
        _assert_422_envelope(exc_info, "abc")

    @pytest.mark.asyncio
    async def test_empty_days_means_unfiltered(self):
        with patch.object(stat_routes, "get_usage_data", new_callable=AsyncMock) as get_usage:
            await stat_routes.stat_usage(users="", models="", days="", _=None)
            get_usage.assert_awaited_once_with([], [], None)

    @pytest.mark.asyncio
    async def test_numeric_days_is_parsed(self):
        with patch.object(stat_routes, "get_usage_data", new_callable=AsyncMock) as get_usage:
            await stat_routes.stat_usage(users="", models="", days="7", _=None)
...
class TestStatSummaryDays:
    ...
    async def test_non_numeric_days_raises_422_envelope_not_500(self):
        with pytest.raises(HTTPException) as exc_info:
            await stat_routes.stat_summary(users="", models="", days="1x", _=None)
        _assert_422_envelope(exc_info, "1x")

    @pytest.mark.asyncio
    async def test_empty_days_means_unfiltered(self):
        with patch.object(stat_routes, "get_summary", new_callable=AsyncMock) as get_summary:
            await stat_routes.stat_summary(users="", models="", days="", _=None)
...
class TestStatRequestsDays:
    ...
    async def test_non_numeric_days_raises_422_envelope_not_500(self):
        with pytest.raises(HTTPException) as exc_info:
            await stat_routes.stat_requests(
    ...
    @pytest.mark.asyncio
    async def test_empty_days_means_unfiltered(self):
        with patch.object(stat_routes, "get_requests", new_callable=AsyncMock) as get_requests:
            await stat_routes.stat_requests(
                users="", models="", providers="", status="all", error_code="",
