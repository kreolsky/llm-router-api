...
**Configuration (YAML)**: Three files in `config/` — `providers.yaml` (connections, incl. the optional per-provider `reasoning_dialect: openai | deepseek | openrouter`, default `openai`, fail-fast at construction on a typo), `models.yaml` (model registry, incl. the optional per-model `reasoning_effort` policy), `user_keys.yaml` (API key access control). Hot-reloaded via background task.
...
2. **Auto-cache** — `src/core/model_capabilities/` `CapabilitiesCache`, persisted to `data/model_cache.json`. A background task (`capabilities_refresh_loop`) calls one `list_models()` per provider, normalizes the raw upstream response (`normalize_provider_model`, shape-dispatched: OpenRouter / llama-server / generic-empty), and stores it. Stale-if-error: on upstream failure the old entry is retained.

Priority: `model_info.yaml` **always wins** over the auto-cache (deep-merge where lists are *replaced*, not concatenated — `merge_capabilities`, distinct from `utils.deep_merge`). The single serializer `render_capabilities()` produces the response shape (derives `supports_vision`, `architecture.modality`, `top_provider`, string `pricing` without exponent). The **hot path never touches the network** — `?refresh=true` is a debug-only best-effort refresh. `reasoning.supported` is auto-derived by the normalizer when upstream `supported_parameters` carries `reasoning` or `reasoning_effort`; `effort_levels`/`default_effort` come from the `models.yaml` policy and `default_enabled` is manual-only — no upstream shape expresses either. `scripts/dump_model_info.py` prints a `model_info.yaml` draft from the cache (stdout only, never writes config).

**Reasoning Effort Policy** (per model, `models.yaml`): an optional `reasoning_effort:` block — `allowed: [low, high, max]`, `param: reasoning_effort | reasoning.effort` (`default:` supported but deliberately unused: the client-side default is the harness's business, not the router's) — gates what effort values may leave for the upstream and injects the default when the client sends none (client value read from both dialects, left where the client put it; a value outside `allowed` is a 400 `invalid_parameter_value`, never a clamp). Enforced in the one funnel (`BaseService._prepare_dispatch` → `src/services/reasoning_effort.py`); `options.reasoning_effort` still wins over the injected default. Soft-validated at load (`ConfigManager._validate_models` — warn + drop the block on a typo; hot-reload safe). `/v1/models` derives `reasoning.effort_levels`/`default_effort` from the same key (beats the auto-cache, loses to `model_info.yaml`).

**Reasoning Dialect Translation** (per provider, `providers.yaml` `reasoning_dialect`, default `openai`): the same funnel, right after the effort policy, re-shapes the harness's `thinking: {type}` / `reasoning_effort` pair per upstream family (`src/services/reasoning_dialect.py`) — `deepseek` keeps both fields verbatim (the native wire shape, also spoken by z.ai); `openai` drops `thinking` (the dialect has no such field) and passes `reasoning_effort` through; `openrouter` re-nests the effort to `reasoning.effort` (vocabulary mapped `max → high`), maps a disabled thinking to `reasoning: {enabled: false}` (title turns), and never gates — the 400-gate stays the effort policy's. Keyed on the provider entry, not the model id, so the same model translates differently when repo and deploy route it to different backends. `GET /v1/capabilities` exposes the flat per-model `{supported, effort_levels}` map from the same derivation `/v1/models` renders (auth parity: `is_hidden` + `allowed_models` + per-key endpoint grants).

## Configuration (env vars)

All env-backed settings are read via `ConfigManager` **once, at construction** into a frozen `Settings` dataclass (`config_manager.settings`, passed to providers as a required argument; each field's env var is its upper-cased name) — env vars cannot change without a restart, so per-access reads only cost the hot path a parse and moved malformed-value failures into requests. No direct `os.getenv` in providers/services except initial logger/debug setup.
